...
residue_counts_interface_ligand_sum <- vector("list", Nsteps) 
...
for (step in 1:Nsteps) {
  ...
  residue_counts_interface_ligand_sum[[step]] <- setNames(rep(0, length(amino_acids)), amino_acids)
}
...
for (s in seq_along(pdbL)) {
  ...
  for (step in 1:Nsteps) {
    ...
    residue_counts_interface_ligand_sum[[step]] <- residue_counts_interface_ligand_sum[[step]] + result$ag_counts[[step]]
  }
}
...
datasets <- list(
  ...
  residue_counts_interface_ligand_df = residue_counts_interface_ligand_sum
)
