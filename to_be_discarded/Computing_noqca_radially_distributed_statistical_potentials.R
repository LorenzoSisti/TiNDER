...
results_dir <- "/Users/lorenzosisti/Downloads/matrici_stratificate_sparse_noqca_17_06/" 
...
for (idx in seq_along(ring_pairs)) {
  ...
  # 1. Calcola i due scenari orientati
  out_12 <- outer(P_sym1, P_sym2, "*") # Ring1(riga) x Ring2(colonna)
  out_21 <- outer(P_sym2, P_sym1, "*") # Ring2(riga) x Ring1(colonna)
  
  # 2. Crea lo stato di riferimento propriamente simmetrizzato
  # Per le celle fuori diagonale fa la media dei due orientamenti.
  # Sulla diagonale (i=j) i due outer sono già identici, la media restituisce il valore corretto.
  expected_sym_corretto <- (out_12 + t(out_21)) / 2
  
  # 3. Calcola il rapporto (ora sia il numeratore che il denominatore sono perfettamente simmetrici)
  ratio_sym <- P_obs_sym / expected_sym_corretto
